...
print_success "Install Required Packages"
}
...
Expired :$exp
──────────────────────────
Copyright By Gemilangkinasih $(date +%Y)
──────────────────────────
...
function pasang_ssl() {
clear
print_install "Install SSL Subdomain Server"
...
print_success "Install SSL Subdomain Server"
}
...
systemctl stop xray
wget -q -O /usr/local/bin/xray "${REPO}files/xray"
chmod +x /usr/local/bin/xray
systemctl restart xray
wget -O /etc/systemd/system/runn.service "${REPO}files/runn.service" >/dev/null 2>&1
print_success "Install Xray"
clear
print_install "Install Package Configuration"
...
EOF
print_success "Install Package Configuration"
}
...
print_success "Install Service Limit IP & Quota" 
}
...
wget -q -O /usr/sbin/dropbear "${REPO}files/dropbear"
...
wget -q ${REPO}files/vnstat-2.6.tar.gz
...
print_success "Install UDP-Custom"
}
...
rm -f /root/key.pem
rm -f /root/cert.pem
print_success "Restarting All Service"
}

function menu(){
clear
print_install "Install Menu Packages"
...
print_success "Install Menu Packages" 
}
